...
for dt in pd.date_range(start_date, periods=23, freq='M'):
	...
	model_3 = AdaBoostRegressor(KNeighborsRegressor(n_neighbors=5, n_jobs=-1), random_state=1234, n_estimators=50)

	print("started model 1")

	model_1.fit(X_train, y_train)

	print("started model 2")
	...
	y3 = model_3.predict(X_stack)

	print("started stack")

	model_stack.fit(np.hstack([X_stack, y1.reshape(-1,1), y3.reshape(-1,1)]), y_stack)
	...
	for r in regions:
		...
		pred_window = calendar.monthrange(dt.year, dt.month)[1]
		...
		for X_test, y_test, d in tqdm(tep.get_day_X_y(limit=n_days, region=r)):
			...
			y_pred_3 = model_3.predict(np.array(X_test))
			...
			y_pred_3[~relevant_mask] = 0

			new = np.hstack([X_test, y_pred_1.reshape(-1,1), y_pred_3.reshape(-1,1)])
